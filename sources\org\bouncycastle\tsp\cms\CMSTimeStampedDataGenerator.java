...
public class CMSTimeStampedDataGenerator extends CMSTimeStampedGenerator {
    ...
    public CMSTimeStampedData generate(TimeStampToken timeStampToken, InputStream inputStream) throws CMSException {
        ...
        BERConstructedOctetString bERConstructedOctetString = byteArrayOutputStream.size() != 0 ? new BERConstructedOctetString(byteArrayOutputStream.toByteArray()) : null;
        TimeStampAndCRL timeStampAndCRL = new TimeStampAndCRL(timeStampToken.toCMSSignedData().getContentInfo());
        if (this.dataUri != null) {
            dERIA5String = new DERIA5String(this.dataUri.toString());
        }
        return new CMSTimeStampedData(new ContentInfo(CMSObjectIdentifiers.timestampedData, new TimeStampedData(dERIA5String, this.metaData, bERConstructedOctetString, new Evidence(new TimeStampTokenEvidence(timeStampAndCRL)))));
    }
