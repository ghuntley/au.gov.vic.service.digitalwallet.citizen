...
public final class zzmh implements zzme {
    ...
    static {
        ...
        zza = zzdm.zza("measurement.ad_id_cache_time", 10000L);
        zzb = zzdm.zza("measurement.config.cache_time", DateFormattingHelper.DAY_IN_MS);
        zzc = zzdm.zza("measurement.log_tag", "FA");
        zzd = zzdm.zza("measurement.config.url_authority", "app-measurement.com");
        zze = zzdm.zza("measurement.config.url_scheme", "https");
        zzf = zzdm.zza("measurement.upload.debug_upload_interval", 1000L);
        zzg = zzdm.zza("measurement.lifetimevalue.max_currency_tracked", 4L);
        zzh = zzdm.zza("measurement.store.max_stored_events_per_app", 100000L);
        zzi = zzdm.zza("measurement.experiment.max_ids", 50L);
        zzj = zzdm.zza("measurement.audience.filter_result_max_count", 200L);
        zzk = zzdm.zza("measurement.alarm_manager.minimum_interval", 60000L);
        zzl = zzdm.zza("measurement.upload.minimum_delay", 500L);
        zzm = zzdm.zza("measurement.monitoring.sample_period_millis", DateFormattingHelper.DAY_IN_MS);
        zzn = zzdm.zza("measurement.upload.realtime_upload_interval", 10000L);
        zzo = zzdm.zza("measurement.upload.refresh_blacklisted_config_interval", RemoteConfigKt.ONE_WEEK);
        zzp = zzdm.zza("measurement.config.cache_time.service", 3600000L);
        zzq = zzdm.zza("measurement.service_client.idle_disconnect_millis", 5000L);
        zzr = zzdm.zza("measurement.log_tag.service", "FA-SVC");
        zzs = zzdm.zza("measurement.upload.stale_data_deletion_interval", DateFormattingHelper.DAY_IN_MS);
        zzt = zzdm.zza("measurement.upload.backoff_period", 43200000L);
        zzu = zzdm.zza("measurement.upload.initial_upload_delay_time", 15000L);
        zzv = zzdm.zza("measurement.upload.interval", 3600000L);
        zzw = zzdm.zza("measurement.upload.max_bundle_size", PlaybackStateCompat.ACTION_PREPARE_FROM_SEARCH);
        zzx = zzdm.zza("measurement.upload.max_bundles", 100L);
        zzy = zzdm.zza("measurement.upload.max_conversions_per_day", 500L);
        zzz = zzdm.zza("measurement.upload.max_error_events_per_day", 1000L);
        zzaa = zzdm.zza("measurement.upload.max_events_per_bundle", 1000L);
        zzab = zzdm.zza("measurement.upload.max_events_per_day", 100000L);
        zzac = zzdm.zza("measurement.upload.max_public_events_per_day", 50000L);
        zzad = zzdm.zza("measurement.upload.max_queue_time", 2419200000L);
        zzae = zzdm.zza("measurement.upload.max_realtime_events_per_day", 10L);
        zzaf = zzdm.zza("measurement.upload.max_batch_size", PlaybackStateCompat.ACTION_PREPARE_FROM_SEARCH);
        zzag = zzdm.zza("measurement.upload.retry_count", 6L);
        zzah = zzdm.zza("measurement.upload.retry_time", 1800000L);
        zzai = zzdm.zza("measurement.upload.url", "https://app-measurement.com/a");
        zzaj = zzdm.zza("measurement.upload.window_interval", 3600000L);
    }
}
