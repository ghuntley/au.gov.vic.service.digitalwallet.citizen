...
public final class SemaphoreImpl implements Semaphore {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:55:0x0075, code lost:
        continue;
     */
    private final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
        Object obj;
        boolean z;
        boolean z2;
        ...
        do {
            ...
            while (true) {
                if (semaphoreSegment2.getId() >= j && !semaphoreSegment2.getRemoved()) {
                    obj = SegmentOrClosed.m1371constructorimpl(semaphoreSegment2);
                    break;
                }
                Object nextOrClosed = semaphoreSegment2.getNextOrClosed();
                if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                    obj = SegmentOrClosed.m1371constructorimpl(ConcurrentLinkedListKt.CLOSED);
                    break;
                }
            ...
            z = true;
            if (SegmentOrClosed.m1376isClosedimpl(obj)) {
                break;
            }
            Segment r6 = SegmentOrClosed.m1374getSegmentimpl(obj);
            while (true) {
                ...
                if (segment.getId() >= r6.getId()) {
                    ...
                    continue;
                    break;
                } else if (tail$FU.compareAndSet(this, segment, r6)) {
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1374getSegmentimpl(obj);
        ...
        if (semaphoreSegment4.acquirers.compareAndSet(i, SemaphoreKt.PERMIT, SemaphoreKt.TAKEN)) {
            ...
            cancellableContinuation.resumeWith(Result.m12constructorimpl(unit));
            return true;
        }
